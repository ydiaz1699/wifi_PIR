...
| U-02 | PIR por flanco y antirrebote compatible | V3, V4, BUG-003 | V4 usa 200 ms por defecto, conserva `antireboteMs` configurable para PIR y sincroniza el nivel inicial antes del primer ciclo | U0 | Emisor final V4 | El valor final es explícito, el nivel presente al arranque no genera un flanco falso, se prueba con HC-SR501 sostenido y no pierde TIMBRE |
| U-03 | TIMBRE activo-bajo con antirrebote | V3 y V4 | V4 usa `ANTIREBOTE_TIMBRE_MS` del perfil de hardware, mantiene semántica lógica `true=presionado` y sincroniza el nivel inicial | U0 | Emisor final V4 | Una pulsación produce un solo evento; otra pulsación posterior produce otro; `STATE_BUTTON=1` significa pulsado |
...
| U-15 | Registry y estados ONLINE/STALE/OFFLINE | V4 `IoTNode` y central | Parcialmente implementado: los estados usan `lastSeen` de cualquier paquete válido y los umbrales canónicos `IOT_STALE_TIMEOUT_MS`/`IOT_OFFLINE_TIMEOUT_MS`; `HEARTBEAT_TIMEOUT_MS` de la central no es una configuración independiente | U0/R1 | `IoTNode` + central | HELLO, heartbeat, cambio de IP y timeout actualizan identidad y disponibilidad correctamente según el contrato elegido; separar liveness por heartbeat requiere diseño y pruebas adicionales |
| U-16 | MQTT fuera del camino crítico UDP | V3 y central V4 | Primer `connect()` diferido al loop después de `node.loop()`; orden real corregido a WiFi → `node.loop()` → buzzer → MQTT; `mqttDisponible` refleja `mqtt.loop()` + `mqtt.connected()` y se limpia al perder WiFi. `PubSubClient::connect()` sigue siendo síncrono durante cada intento | U0/R1 | Adapter MQTT central | Un evento UDP recibido al arranque se procesa/ACK/ejecuta antes del primer intento MQTT; la pérdida de broker no deja disponibilidad stale; bloqueo de `connect()` requiere integración/hardware |
| U-17 | Buzzer y alarma PIR/TIMBRE | `receptor_bocina`, central V4 | V4 conserva MOTION/TIMBRE, modos, publicaciones V3 y duraciones 1000/500 ms; temporizador rollover-safe y orden de loop corregido. La activación usa allowlist: UNKNOWN, cierres/liberaciones, LOW_BATTERY y TAMPER no activan bocina hasta tener contrato explícito; STATE_ALARM/FLOOD ya tienen traducción MQTT | U0 | Perfil alarma central | MOTION solo suena armado, TIMBRE suena también desarmado, cada evento publica su payload V3 y el temporizador no bloquea; eventos no autorizados acústicamente no suenan; política TAMPER sigue pendiente de aprobación |
...
| U-23 | Configuración remota autenticada | `IoTConfigHandler` | Handler emisor; RESPONSE se genera antes del callback que cambia auth, por lo que la transición usa deliberadamente la política anterior. No hay flujo completo central CONFIG/RESPONSE ni `CMD_ID` de configuración | R1 | módulo CONFIG/RESPONSE | CONFIG válida se aplica atómicamente; la respuesta se correlaciona y se verifica con una política de transición definida; no rotar HMAC en el mismo paso sin handshake |
